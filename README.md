...
| `weka_org_admin_secret` | `String (secret)` | **REQUIRED** | HMAC key used to derive per-organization admin passwords deterministically |
...
| manage_existing | ✗ | ✓ | WEKAFS unsupported under mandatory isolation (the FS cannot be moved into the project org); NFS only |
| unmanage | ✓ | ✓ | |
| get_share_stats | ✓ | ✓ | |
...
> WEKAFS filesystems are created with `auth_required=True` inside that
...
Every Manila project (`project_id`) is mapped to its own Weka
**organization**. WEKAFS filesystems for that project are created inside
the organization with `auth_required=True`, giving true cross-tenant
isolation: a token scoped to project A's organization cannot mount
project B's shares — the Weka cluster enforces this regardless of
network reachability. This behavior is mandatory and requires a Weka
cluster with multi-tenant organization support (5.x REST v2).
...
stored; this option is required and the driver refuses to start without
it). Two principals exist per org: a **TenantAdmin** the driver uses
