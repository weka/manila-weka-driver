...
- `update_access` honors both contract modes. Incremental: apply `add_rules`/`update_rules`, remove `delete_rules`. Full sync (all three empty — recovery/resync): apply `access_rules` **and reconcile**, i.e. prune backend state no rule maps to. `_prune_nfs_rules` sweeps export permissions on the share's fs whose `manila-<share8>-<rule8>` client group is unexpected (then deletes the group); `_prune_wekafs_rules` drops unexpected IPs from the per-share `manila-<share8>-rw|ro` policies via `_policy_set_ip(present=False)`, which detaches+deletes a policy once empty. Model B (`weka:security_policy_group`) is exempt — the shared group policies, not per-share rules, are the source of truth. Without the sweep, access revoked while the share manager was down would stay live on the cluster.
