...
weka_opts = [
    ...
    cfg.BoolOpt(
        'weka_wekafs_isolation',
        default=True,
        help=(
            'Enforce per-tenant isolation for WEKAFS shares. When True '
            '(default), each Manila project is mapped to a dedicated Weka '
            'organization and its filesystems are created with '
            'authentication required, so a client can only mount a share '
            'if it holds a token scoped to that project\'s organization. '
            'When False, WEKAFS filesystems are created in the '
            'weka_organization with authentication disabled (legacy '
            'behaviour: any host on the Weka fabric can mount). NFS shares '
    ...
    cfg.StrOpt(
        ...
        help=(
            ...
            'Weka organization admin is derived deterministically as '
            'HMAC-SHA256(weka_org_admin_secret, project_id), so no '
            'per-tenant secret is stored by the driver. Required when '
            'weka_wekafs_isolation is True. Keep this value stable: '
            'rotating it invalidates the logins of existing organizations.'
        ),
    ),
    cfg.StrOpt(
        'weka_org_prefix',
