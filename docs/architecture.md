...
- Export path: `<weka_api_server>/<fs_name>`
- With isolation enabled (default), the filesystem is created inside the
  project's Weka org with `auth_required=True`; only a client holding a
  token scoped to that org can mount it.  See §10 below.
...
When `weka_wekafs_isolation = true` (default), each Manila project gets
its own Weka organization (tenant):
