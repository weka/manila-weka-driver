...
### Users
- GET/POST `/users` (D) — POST called twice during org provisioning:
  once for the TenantAdmin user (role: TenantAdmin) and once for the
  Regular mount user (role: Regular, username: `<weka_org_user>-mnt`);
  also called by `update_access` (WEKAFS) to ensure the mount user
  exists (idempotent)
- DELETE `/users/{uid}` (S)
